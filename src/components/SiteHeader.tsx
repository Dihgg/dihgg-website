...
export default function SiteHeader({ brandHref, blogHref, contactHref, blogLabel, contactLabel }: Props) {
  return (
    <header className="sticky top-0 z-50 border-b border-blue-100/80 bg-white/80 backdrop-blur-md">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-6 py-3">
        ...
        <nav className="flex items-center gap-2" aria-label="Main navigation">
          <a
            href={blogHref}
            className="rounded-full border border-blue-100 bg-white/90 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-blue-50 hover:text-blue-700"
          >
            {blogLabel}
          </a>
          <a
