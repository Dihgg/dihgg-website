...
```mermaid
flowchart LR
    H[Human] -->|Questions| M[Machine]
    H -->|Questions| P[Person]
    M -->|Answers| H
    P -->|Answers| H
```
...
```mermaid
flowchart TD
    Input[Chinese text] --> Person
    Person --> Rules[Rulebook]
    Rules --> Person
    Person --> Output[Correct Chinese response]
```
...
```mermaid
flowchart LR
    Data[Large-scale training] --> Model
    Model --> Tokens[Token prediction]
    Tokens --> Text[Coherent text]
```
