...
```mermaid
flowchart LR
    H[Humano] -->|Perguntas| M[Máquina]
    H -->|Perguntas| P[Pessoa]
    M -->|Respostas| H
    P -->|Respostas| H
```
...
```mermaid
flowchart TD
    Input[Texto em Chinês] --> Pessoa
    Pessoa --> Manual[Regras]
    Manual --> Pessoa
    Pessoa --> Output[Resposta correta em Chinês]
```
...
```mermaid
flowchart LR
    Dados[Treino em larga escala] --> Modelo
    Modelo --> Tokens[Previsão de tokens]
    Tokens --> Texto[Texto coerente]
```
